...
**这一层扫描一定跑到末尾，没有键数上限。** 目录与计数是用户判断"库里有什么"和"这个目录删掉会怎样"的依据，所以提前收手等于给出错的下限、还可能整个漏掉一个目录。曾有一个 20 万键的上限，结果 48.5 万键的库上报「该库共 485,073 个键，此处仅扫描了 200,012 个：目录可能不全，计数是下限」——那是对缺陷的准确描述，但用户要的不是警告，是完整结果。现在约束来自**库的大小**而不是常量：走这条路的前提是库小于 100 万键（见下），这个规模完整扫一遍是几秒的事。

实测（本机 Docker Redis）：

| 操作 | 耗时 |
| --- | --- |
| 打开 26.5 万键库的根层（9 个目录 + 精确计数） | 0.7–1.4 s |
| 展开一个 2 万键的分支 | 0.4 s |
| 展开 `goods:item`（**20 万键在同一层**） | 1.0 s |
| 完整扫描 48.5 万键的库（19 ms RTT） | 约 10 s |

单层行数有上限（5000），超出的一页不发。这个上限只限制**显示**，与目录完整性无关——后者绝不能有。上限本身不再附带任何文字说明：屏幕上那几行就是答案，在下面再写一句"该层共 N 个键，仅显示前 M 个"只是把目录树变成了一块需要读说明的面板。目录树只保留过程提示（正在扫描、索引构建中）和结果提示（找到 N 个键），不写解释性长句。
...
- **小库不建索引**：100 万键以下逐层扫描会完整扫完该层，不必让只想看几千个键的人付出前置成本；48.5 万键的库逐层扫描一次约 10 s，仍在一次点击可接受的范围内。
- **大库直接建，不弹确认**：展开 100 万键以上的库就直接开始遍历，边建边看（顶部有真实进度条），不做前置确认。曾经有个确认框，把键数与预计耗时摆出来让用户自己权衡——但那是让用户去替面板判断一个面板已经量过的数字，而这个操作是只读的、分批推进的，真正的备选方案只有"干脆不浏览这个库"。进度条是用户实际需要的，确认框只是挡在他要的那件事前面。
- **进度是真实工作量**：显示 `已扫描 X / Y，发现 N 个目录` 加进度条，而不是未知时长的转圈。索引还没建完就展开层级，该层会显示「索引构建中：已扫描 M / N，此层的目录与计数会随进度补齐」——这是**临时状态**，建完自动消失。
- **不留漏目录的可能**：索引是完整遍历，不是采样。实测一个 1950 万键的库里存在**只有 1 个键**的目录（`hyperf`、`shopadmin`），采样会漏掉它们——而目录列表正是用户判断"库里有什么"的依据。同样的理由，目录总数超过索引上限（20 万）时**报错而不是静默丢目录**：宁可说"这个 keyspace 建不了索引，请改用过滤或窄目录"，也不给一份看着完整、实则缺项的目录。
...
| `scripts/e2e-index-routes.mjs` | 索引路由端到端：遍历完整、层级与直接扫描一致、缓存不重扫、失效后可重建 |
