...
- **执行后把结果滚动进视口。** 字段全显示之后，宽表的结果区落在折叠线下方，不滚动的话用户按了 `执行` 画面不动，会以为没反应（搜索其实跑了）。用 `scrollIntoView({ block: 'nearest' })`：结果已在屏幕上时不动。
...
| `scripts/e2e-sql-search.mjs` | 搜索页（QBE）端到端（21 项）：用 `Input.insertText` 走浏览器真实输入；断言表头就是 字段/类型/排序规则/运算符/值、**每一列都有自己的一行**、**空表单查全部**（4 行而不是报错）、两行填值按 AND 组合、绑定语义（`%` 是字符、`' OR 1=1 --` 匹配 0 行）、`is NULL` 只回 NULL 行。几何断言分两组：**字段列表完整显示且自身无滚动框**（用一个 25 列表格量，3 列的表量不出来——旧缺陷的框里装得下 3 行，所有断言都会通过）、以及**`执行` 在顶部/中部/底部三个滚动位置都可见**（这两个要求互相拉扯，必须一起断言） |
