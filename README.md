...
- 右侧标签页：**浏览**（分页与跳页、点列排序、按索引排序、双击单元格就地编辑、行编辑/复制/删除、多选批量删除；也用于承载 SQL 查询的结果集）、**结构**（改列/删列/新增列、主键编辑、唯一约束、索引管理、非重复值计数）、**SQL**（编辑器，Ctrl+Enter 执行；SELECT 的结果见浏览页）、**搜索**（phpMyAdmin 的依例查询：每列一行，填了值的行才参与，见下）、**插入**（按列类型给控件）、**操作**（照 phpMyAdmin 的操作页：移动表 / 表选项 / 复制表 / 表维护 / 删除数据或表）
