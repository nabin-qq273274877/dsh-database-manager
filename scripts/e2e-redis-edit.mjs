...
const FLOW = `(async () => {
  ...
  // Open the target database. Nothing is expanded on entry, so this is normally
  // just the one click; the loop closes any other database the script opened
  // earlier, so the tree has exactly one branch open when the assertions run.
