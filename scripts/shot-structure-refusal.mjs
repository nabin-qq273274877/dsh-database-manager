...
/*
 * Screenshot a REFUSED column-editor submit, so the error's placement can be read by eye.
 *
 * The E2E script asserts the geometry (inside the dialog box, inside the viewport); this is
 * for the other question a geometry assertion answers badly — does it LOOK like part of the
 * dialog, or like a banner that happens to overlap it.
 *
 * Usage: node scripts/shot-structure-refusal.mjs <baseUrl-with-token> <sqliteFile>
 */
...
if (baseUrl === undefined || sqliteFile === undefined) {
  console.error('usage: node scripts/shot-structure-refusal.mjs <baseUrl-with-token> <sqliteFile>')
  process.exit(2)
}

const OUT = join(process.cwd(), '.shots')
const TAG = process.pid
const SOURCE_NAME = `Shot Refusal ${TAG}`

function seed(file) {
  const db = new DatabaseSync(file)
  db.exec('DROP TABLE IF EXISTS refusal_target')
  db.exec('CREATE TABLE refusal_target (id INTEGER PRIMARY KEY, a TEXT, b TEXT)')
  db.exec("INSERT INTO refusal_target (id, a, b) VALUES (1, 'x', 'y')")
  db.close()
}
...
const profile = mkdtempSync(join(tmpdir(), 'dbm-shotref-'))
...
try {
  ...
  click(dbNode);
  await sleep(1800);
  const tableLink = await waitFor(() => byText('.dbm-table .dbm-link', 'refusal_target'), 12000);
  ...
  await sleep(500);
  // Submit with an EMPTY name: a local refusal, so no server round trip is needed.
  click(document.querySelector('[data-dbm-column-submit]'));
  await sleep(1200);
  return { ok: true, error: (document.querySelector('.dbm-modal .dbm-error') || {}).textContent };
} catch (error) {
  ...
  writeFileSync(join(OUT, 'structure-refusal.png'), Buffer.from(result.data, 'base64'))
  console.log(`wrote .shots/structure-refusal.png (error text: ${JSON.stringify(ready.error)})`)
} finally {
  try {
