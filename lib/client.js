window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
		  ...
		  "structure.lengthInvalid": "\u5217\u300C{name}\u300D\u7684\u957F\u5EA6/\u503C\u4E0D\u5408\u6CD5\uFF1A\u53EA\u80FD\u586B\u6570\u5B57\uFF08\u53EF\u542B\u4E00\u4F4D\u9017\u53F7\uFF09\u6216\u5E26\u5F15\u53F7\u7684\u503C\u5217\u8868",
		...
		var en = {
		  ...
		  "structure.lengthInvalid": "The length/values for \u201C{name}\u201D are not valid: use digits (with at most one comma) or a quoted list of values",
		...
		function SqlStructureTab(props) {
		  ...
		  const submitEditor = async () => {
		    ...
		    if (type === "") {
		      onError(t("structure.lengthRequired", { type: draft.type || t("common.none"), column: name }));
		      return;
		    }
		  ...
		  const runIndexPlan = async (plan) => {
		    for (const operation of plan) {
		      const done = operation.op === "setPrimaryKey" ? await run({ action: "setPrimaryKey", columns: operation.columns }, { keepEditor: true }) : operation.op === "dropIndex" ? await run({ action: "dropIndex", name: operation.name }, { rows: false, keepEditor: true }) : await run(
