window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function RedisDatabaseView(props) {
		  ...
		  const refreshAll = React5.useCallback(async () => {
		    try {
		  ...
		  for (const schema of schemas) {
		    ...
		    tree.push(
		      React8.createElement(
		        ...
		        tables === void 0 ? null : React8.createElement("span", { className: "dbm-tree-meta" }, String(tables.filter((table) => matchesFilter2(table.name)).length))
		      )
		    );
		    if (!isOpen) continue;
