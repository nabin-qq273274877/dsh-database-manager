window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var DbApi = class {
		  ...
		  async redisTree(id, options) {
		    return (await readJson(await fetch(DB_API.redisTree(id, query(options))))).page;
		  }
		...
		function filterTree(tree, filter) {
		  ...
		  const prune = (node) => {
		    ...
		    for (const child of node.folders) {
		      const kept2 = prune(child);
		      if (kept2 !== void 0) folders.push(kept2);
		    }
		    const keys = node.keys.filter((info) => matchesFilter(info.key, filter));
		    if (folders.length === 0 && keys.length === 0) return void 0;
		    return { ...node, folders, keys };
		  };
		  const root = { name: "", path: "", folders: tree.folders, keys: tree.keys };
		...
		function RedisKeyTree(props) {
		  ...
		  const renderFolder = (db, folder, depth) => {
		    ...
		    const rows2 = [
		      React2.createElement(
		        ...
		        React2.createElement("span", { className: "dbm-tree-meta" }, t("redisdb.keyCount", { n: countFolderKeys(folder) })),
		        React2.createElement(
		          "span",
		          { className: "dbm-tree-actions" },
		          React2.createElement(
		    ...
		    if (!isOpen) return rows2;
		    for (const child of folder.folders) rows2.push(...renderFolder(db, child, depth + 1));
		    for (const info of folder.keys) rows2.push(renderKey(db, info, depth + 1));
		    return rows2;
		  };
		  const rows = [];
		  for (const node of databases) {
		    ...
		    rows.push(
		      React2.createElement(
		        ...
		        React2.createElement("span", { className: "dbm-tree-meta" }, t("redisdb.keyCount", { n: node.keys })),
		        React2.createElement(
		          "span",
		          { className: "dbm-tree-actions" },
		          React2.createElement(
		    ...
		    if (!isOpen) continue;
		    if (tree === void 0) {
		      rows.push(React2.createElement("div", {
		        key: `loading-${node.db}`,
		...
		function RedisDatabaseView(props) {
		  ...
		  const [info, setInfo] = React3.useState(initialInfo);
		  const [filter, setFilter] = React3.useState("");
		  const [trees, setTrees] = React3.useState({});
		  const [loadingDbs, setLoadingDbs] = React3.useState({});
		  const [openDbs, setOpenDbs] = React3.useState(() => ({ [source.db ?? 0]: true }));
		  const [openFolders, setOpenFolders] = React3.useState({});
		  const [selection, setSelection] = React3.useState(void 0);
		  const [activeKey, setActiveKey] = React3.useState(void 0);
		  const [value, setValue] = React3.useState(void 0);
		  const [valueLoading, setValueLoading] = React3.useState(false);
		  const [tab, setTab] = React3.useState("value");
		  const [error, setError] = React3.useState(void 0);
		  const [notice, setNotice] = React3.useState(void 0);
		  const [createFor, setCreateFor] = React3.useState(void 0);
		  const [deleteFor, setDeleteFor] = React3.useState(void 0);
		  const [busy, setBusy] = React3.useState(false);
		  const loadTree = React3.useCallback(async (db) => {
		    setLoadingDbs((current) => ({ ...current, [db]: true }));
		    try {
		      const page = await api.redisTree(source.id, { db });
		  ...
		  const refreshAll = React3.useCallback(async () => {
		    try {
		  ...
		  React3.useEffect(() => {
		    void loadTree(initialDb);
		  }, [initialDb, loadTree]);
		  const toggleDb = (db) => {
		    const next = openDbs[db] !== true;
		    setOpenDbs((current) => ({ ...current, [db]: next }));
		    if (next && trees[db] === void 0) void loadTree(db);
		  };
		  ...
		  const loadValue = React3.useCallback(async (db, key) => {
		  ...
		  const databases = React3.useMemo(() => {
		  ...
		  const visibleTrees = React3.useMemo(() => {
		    const next = {};
		    for (const [key, tree] of Object.entries(trees)) {
		      next[Number(key)] = tree === void 0 ? void 0 : filterTree(tree, filter);
		    }
		    return next;
		  }, [trees, filter]);
		  ...
		  const confirmDelete = async () => {
		    ...
		    try {
		      if (deleteFor.target.kind === "key") {
		        ...
		        setNotice(result.truncated ? `${t("sql.affected", { n: result.deleted, ms: 0 })} \xB7 ${t("redisdelete.truncated")}` : t("sql.affected", { n: result.deleted, ms: 0 }));
		  ...
		  const left = React3.createElement(
		    "div",
		    { className: "dbm-side" },
		    ...
		    React3.createElement(
		      ...
		      React3.createElement(RedisKeyTree, {
		        databases,
		        trees: visibleTrees,
		        loadingDbs,
		        openDbs,
		  ...
		  if (tab === "value") {
		    body.push(
		      value === void 0 ? React3.createElement(Empty, { key: "empty", message: valueLoading ? t("common.loading") : t("redis.selectKey") }) : React3.createElement(ValueView, { key: "value", value, source })
		    );
		  }
		  if (tab === "info") {
		    body.push(React3.createElement(InfoView, {
		      key: "info",
		      info,
		      onRefresh: () => {
		        void refreshAll();
		  ...
		  if (tab === "console") {
		    body.push(
		      React3.createElement(ConsoleView, {
		        key: "console",
		        api,
		        source,
		  ...
		  return React3.createElement(
		    ...
		    React3.createElement(
		      ...
		      React3.createElement(BackButton, { onBack, label: t("panel.backToList") }),
		      React3.createElement("span", { className: "dbm-title" }, source.name),
		      React3.createElement("span", { className: "dbm-badge dbm-badge-redis" }, "redis"),
		      React3.createElement("span", { className: "dbm-subtitle dbm-mono" }, `${source.host ?? ""}:${source.port ?? ""}/db${activeDb}`),
		      React3.createElement("span", { className: "dbm-spacer" }),
		      React3.createElement(BackButton, { onBack: onClose })
		    ),
		    error === void 0 ? null : React3.createElement(ErrorBanner, { message: error }),
		    notice === void 0 ? null : React3.createElement("div", { className: "dbm-ok", style: { padding: "6px 14px" } }, notice),
		    React3.createElement("div", { className: "dbm-split" }, left, React3.createElement("div", { className: "dbm-main" }, body)),
		    createFor === void 0 ? null : React3.createElement(NewKeyDialog, {
		      db: createFor.db,
		    ...
		    deleteFor === void 0 ? null : React3.createElement(DeleteDialog, {
		...
		function ValueView(props) {
		  ...
		  const head = React3.createElement(
		    ...
		    React3.createElement(
		      "div",
		      { className: "dbm-row" },
		      React3.createElement("span", { className: `dbm-badge dbm-badge-${value.type === "string" ? "ok" : "sqlite"}` }, value.type),
		    ...
		    return React3.createElement(
		      ...
		      React3.createElement(
		        ...
		        React3.createElement(
		          "div",
		          { className: "dbm-pad" },
		          React3.createElement("div", { className: "dbm-hint" }, `${t("redis.stringValue")} \xB7 ${formatBytes((value.value ?? "").length)}`),
		          React3.createElement("pre", { className: "dbm-mono", style: { whiteSpace: "pre-wrap", wordBreak: "break-all", margin: 0 } }, value.value ?? "")
		        )
		      )
		    );
		  }
		  ...
		  return React3.createElement(
		    ...
		    React3.createElement(
		      "div",
		      { className: "dbm-data" },
		      rows.length === 0 ? React3.createElement(Empty, { message: t("redis.noKeys") }) : React3.createElement(
		        "table",
		        null,
		        React3.createElement(
		          "thead",
		          ...
		          React3.createElement(
		            "tr",
		            null,
		            React3.createElement("th", null, firstLabel),
		            React3.createElement("th", null, t("redis.value")),
		            isZset ? React3.createElement("th", null, t("redis.score")) : null
		          )
		        ),
		        React3.createElement(
		          "tbody",
		          null,
		          rows.map(
		            ([left, right, third], index) => React3.createElement(
		              "tr",
		              { key: index },
		              React3.createElement("td", { title: left }, left),
		              React3.createElement("td", { title: right }, right),
		              isZset ? React3.createElement("td", null, third ?? "") : null
		            )
		          )
		        )
		      )
		    ...
		    React3.createElement(
		      ...
		      React3.createElement(
		        ...
		        React3.createElement(
		          "div",
		          { className: "dbm-row" },
		          React3.createElement("strong", null, t("redis.info")),
		          React3.createElement("span", { className: "dbm-spacer" }),
		          React3.createElement("button", { type: "button", className: "dbm-btn dbm-btn-sm", onClick: onRefresh }, t("common.refresh"))
		        ),
		        React3.createElement(
		          "div",
		          { className: "dbm-grid" },
		          facts.map(
		            ([label, val]) => React3.createElement(
		              "div",
		              { key: label },
		              React3.createElement("div", { className: "dbm-hint" }, label),
		              React3.createElement("div", { className: "dbm-mono" }, val)
		            )
		          )
		        ),
		        React3.createElement("strong", null, t("redis.databases")),
		        info.databases.length === 0 ? React3.createElement("div", { className: "dbm-hint" }, t("redis.noKeys")) : React3.createElement(
		          "table",
		          { className: "dbm-table" },
		          React3.createElement("thead", null, React3.createElement("tr", null, React3.createElement("th", null, t("redis.db")), React3.createElement("th", null, t("redis.keys")))),
		          React3.createElement(
		            "tbody",
		            null,
		            info.databases.map(
		              (entry) => React3.createElement(
		                "tr",
		                { key: entry.db },
		                React3.createElement("td", null, `db${entry.db}`),
		                React3.createElement("td", null, String(entry.keys))
		              )
		            )
		          )
		        )
		...
		function ConsoleView(props) {
		  ...
		  const [command, setCommand] = React3.useState("");
		  const [allowWrite, setAllowWrite] = React3.useState(false);
		  const [result, setResult] = React3.useState(void 0);
		  const [busy, setBusy] = React3.useState(false);
		  const run = async () => {
		  ...
		  return React3.createElement(
		    ...
		    React3.createElement(
		      ...
		      React3.createElement(
		        "div",
		        { className: "dbm-row" },
		        React3.createElement("button", { type: "button", className: "dbm-btn dbm-btn-primary", disabled: busy, onClick: () => {
		          void run();
		        ...
		        React3.createElement("span", { className: "dbm-hint" }, `db${db} \xB7 ${allowWrite ? t("redis.allowWrite.hint") : t("redis.readonlyNotice")}`)
		      )
		    ),
		    result === void 0 ? null : React3.createElement(
		      "div",
		      { className: "dbm-tab-body", style: { minHeight: 0 } },
		      React3.createElement("div", { className: "dbm-pad dbm-hint" }, result.message),
		      React3.createElement(
		        "div",
		        { className: "dbm-data" },
		        React3.createElement(
		          "table",
		          null,
		          React3.createElement("thead", null, React3.createElement("tr", null, result.columns.map((column) => React3.createElement("th", { key: column }, column)))),
		          React3.createElement(
		            "tbody",
		            null,
		            result.rows.map(
		              (row, index) => React3.createElement(
		                "tr",
		                { key: index },
		                row.map(
		                  (cell, cellIndex) => React3.createElement(
		                    "td",
		...
		function SourceFormDialog(props) {
		  ...
		  const [state, setState] = React4.useState(() => initialState(source, initialKind ?? "sqlite"));
		  ...
		  const [testResult, setTestResult] = React4.useState(void 0);
		  const patch = (next) => {
		  ...
		  const field = (label, control, key) => React4.createElement("label", { className: "dbm-label", key: key ?? label }, React4.createElement("span", null, label), control);
		  const input = (value, onChange, extra = {}) => React4.createElement("input", {
		    className: "dbm-input",
		    value,
		  ...
		  children.push(
		    field(
		      ...
		      React4.createElement(
		        ...
		        DB_KINDS.map((kind) => React4.createElement("option", { key: kind, value: kind }, kind))
		      ),
		      "kind"
		    )
		  );
		  if (isEdit) {
		    children.push(React4.createElement("div", { className: "dbm-hint", key: "kind-hint" }, t("form.kindLocked")));
		  }
		  children.push(
		    React4.createElement(
		      "div",
		      ...
		      isEdit ? field(t("form.id"), React4.createElement("div", { className: "dbm-mono" }, state.id), "id") : field(t("form.id"), input(state.id, (value) => patch({ id: value }), { placeholder: t("form.id.placeholder") }), "id"),
		  ...
		  if (state.kind === "sqlite") {
		    ...
		    children.push(React4.createElement("div", { className: "dbm-grid", key: "connection" }, connection));
		    if (state.kind === "mysql") {
		      children.push(React4.createElement("div", { className: "dbm-hint", key: "schemaless-hint" }, t("form.mysql.noDefaultSchema")));
		    }
		    children.push(
		      field(
		        t("form.password"),
		    ...
		    children.push(React4.createElement("div", { className: "dbm-hint", key: "tls-hint" }, t("form.tls.hint")));
		    children.push(
		      React4.createElement(
		        "div",
		        { className: "dbm-field-inline", key: "timeout" },
		        React4.createElement("span", null, t("form.timeout")),
		        React4.createElement("input", {
		          className: "dbm-input",
		        ...
		        React4.createElement("span", { className: "dbm-hint" }, t("form.timeout.unit"))
		      )
		    );
		  }
		  children.push(
		  ...
		  children.push(React4.createElement("div", { className: "dbm-hint", key: "readonly-hint" }, t("form.readonly.hint")));
		  children.push(
		    field(
		      t("form.description"),
		  ...
		  if (error !== void 0) children.push(React4.createElement(ErrorBanner, { key: "error", message: error }));
		  if (testResult !== void 0) {
		    children.push(
		      testResult.ok ? React4.createElement(
		        "div",
		        { className: "dbm-ok", key: "test-ok" },
		        t("form.test.ok", { ms: testResult.latencyMs ?? 0, version: testResult.serverVersion ?? "" }).trim(),
		        testResult.note === void 0 ? null : React4.createElement("div", { className: "dbm-hint" }, t("form.test.note", { note: testResult.note }))
		      ) : React4.createElement(ErrorBanner, { key: "test-fail", message: t("test.fail", { error: testResult.error ?? "" }) })
		    );
		  }
		  const footerBusy = busy || testing;
		  return React4.createElement(Modal, {
		    ...
		    footer: React4.createElement(
		      ...
		      React4.createElement(
		        ...
		        onTest === void 0 ? null : React4.createElement(
		          "button",
		          {
		            key: "test",
		            type: "button",
		      ...
		      React4.createElement(
		        ...
		        React4.createElement("button", { key: "cancel", type: "button", className: "dbm-btn", disabled: footerBusy, onClick: onClose }, t("form.cancel")),
		        React4.createElement("button", { key: "save", type: "button", className: "dbm-btn dbm-btn-primary", disabled: footerBusy, onClick: () => {
		          void submit();
		...
		function SourceListView(props) {
		  ...
		  const [groupMode, setGroupMode] = React5.useState("none");
		  const [tests, setTests] = React5.useState({});
		  const [error, setError] = React5.useState(void 0);
		  const [editing, setEditing] = React5.useState(void 0);
		  const [creating, setCreating] = React5.useState(false);
		  const [deleting, setDeleting] = React5.useState(void 0);
		    ...
		    for (const source of group.items) {
		      ...
		      rows.push(
		        React5.createElement(
		          "tr",
		          { key: source.id },
		          React5.createElement("td", null, React5.createElement("span", { className: `dbm-badge dbm-badge-${source.kind}` }, source.kind)),
		          React5.createElement(
		            "td",
		            null,
		            React5.createElement("div", { style: { fontWeight: 500 } }, source.name),
		            source.description === "" ? null : React5.createElement("div", { className: "dbm-hint" }, source.description),
		            React5.createElement("div", { className: "dbm-hint dbm-mono" }, source.id)
		          ),
		          React5.createElement("td", { className: "dbm-mono" }, hostOf(source)),
		          React5.createElement("td", null, source.kind === "mysql" ? source.user ?? t("common.none") : t("common.none")),
		          React5.createElement(
		            "td",
		            null,
		            authOf(source),
		            source.readonly ? React5.createElement("span", { className: "dbm-badge", style: { marginLeft: 6 } }, t("form.readonly")) : null
		          ),
		          React5.createElement(
		            "td",
		            null,
		            source.tags.length === 0 ? t("common.none") : React5.createElement(
		              "span",
		              { className: "dbm-tags" },
		              source.tags.map((tag) => React5.createElement("span", { key: tag, className: "dbm-badge" }, tag))
		            )
		          ),
		          React5.createElement(
		            "td",
		            ...
		            React5.createElement(
		              ...
		              React5.createElement("button", { type: "button", className: "dbm-btn dbm-btn-sm dbm-btn-primary", onClick: () => onConnect(source) }, t("action.connect")),
		              React5.createElement("button", { type: "button", className: "dbm-btn dbm-btn-sm", onClick: () => setEditing(source) }, t("action.edit")),
		              React5.createElement("button", { type: "button", className: "dbm-btn dbm-btn-sm dbm-btn-danger", onClick: () => setDeleting(source) }, t("action.delete"))
		            ),
		  ...
		  const children = [
		    React5.createElement(
		      ...
		      React5.createElement(
		        ...
		        React5.createElement("option", { value: "none" }, `${t("list.groupBy")}: ${t("list.group.none")}`),
		        React5.createElement("option", { value: "kind" }, `${t("list.groupBy")}: ${t("list.group.kind")}`),
		        React5.createElement("option", { value: "group" }, `${t("list.groupBy")}: ${t("list.group.group")}`),
		        React5.createElement("option", { value: "tag" }, `${t("list.groupBy")}: ${t("list.group.tag")}`)
		      ),
		      React5.createElement("span", { className: "dbm-hint" }, t("list.count", { n: filtered.length })),
		      React5.createElement("span", { className: "dbm-spacer" }),
		      React5.createElement("button", { type: "button", className: "dbm-btn dbm-btn-primary", onClick: () => setCreating(true) }, `+ ${t("list.new")}`)
		    )
		  ];
		  if (error !== void 0) {
		    children.push(React5.createElement(ErrorBanner, { key: "error", message: error }));
		  }
		  ...
		  children.push(
		    React5.createElement(
		      ...
		      sources.length === 0 ? React5.createElement(Empty, { message: t("list.empty") }) : filtered.length === 0 ? React5.createElement(Empty, { message: t("list.emptyFiltered") }) : React5.createElement(
		        "table",
		        { className: "dbm-table" },
		        React5.createElement(
		          "thead",
		        ...
		        React5.createElement("tbody", null, rows)
		      )
		    )
		  );
		  ...
		  if (creating || editing !== void 0) {
		    children.push(
		      React5.createElement(SourceFormDialog, {
		        key: "form",
		  ...
		  if (deleting !== void 0) {
		    children.push(
		      React5.createElement(Modal, {
		        key: "delete",
		        title: t("delete.title"),
		        onClose: () => setDeleting(void 0),
		        footer: [
		          React5.createElement("button", { key: "cancel", type: "button", className: "dbm-btn", onClick: () => setDeleting(void 0) }, t("common.cancel")),
		          React5.createElement("button", { key: "ok", type: "button", className: "dbm-btn dbm-btn-danger", onClick: () => {
		            void remove(deleting);
		          } }, t("delete.confirm"))
		        ],
		        children: React5.createElement("div", null, t("delete.body", { name: deleting.name }))
		      })
		    );
		  }
		  return React5.createElement(
		    ...
		    React5.createElement(
		      "div",
		      { className: "dbm-header" },
		      React5.createElement(BackButton, { onBack: props.onBack }),
		      React5.createElement("span", { className: "dbm-title" }, t("panel.title")),
		      React5.createElement("span", { className: "dbm-subtitle" }, t("panel.subtitle")),
		      React5.createElement("span", { className: "dbm-spacer" }),
		      React5.createElement(
		        "span",
		        { className: "dbm-hint" },
		...
		function TestOutcome(state) {
		  if (state.status === "running") {
		    return React5.createElement("div", { className: "dbm-hint" }, t("common.loading"));
		  }
		  const { result } = state;
		  if (result.ok) {
		    return React5.createElement(
		  ...
		  return React5.createElement("div", { className: "dbm-error" }, t("test.fail", { error: result.error ?? "" }));
		}
		function GateStrip(props) {
		  const [busy, setBusy] = React5.useState(false);
		  const [savedAt, setSavedAt] = React5.useState(void 0);
		  ...
		  return React5.createElement(
		    ...
		    React5.createElement("span", { className: "dbm-spacer" }),
		    React5.createElement("span", { className: "dbm-hint" }, savedAt === void 0 ? t("gate.hint") : t("gate.saved"))
		  );
		}
		...
		function SqlDatabaseView(props) {
		  ...
		  const [schemas, setSchemas] = React6.useState(initialSchemas);
		  const [tableFilter, setTableFilter] = React6.useState("");
		  const [openSchemas, setOpenSchemas] = React6.useState({});
		  const [tablesBySchema, setTablesBySchema] = React6.useState({});
		  const [loadingSchemas, setLoadingSchemas] = React6.useState({});
		  const [activeSchema, setActiveSchema] = React6.useState(void 0);
		  const [activeTable, setActiveTable] = React6.useState(void 0);
		  const [tab, setTab] = React6.useState("browse");
		  const [error, setError] = React6.useState(void 0);
		  const [notice, setNotice] = React6.useState(void 0);
		  const [rows, setRows] = React6.useState(void 0);
		  const [page, setPage] = React6.useState(1);
		  const [pageSize, setPageSize] = React6.useState(200);
		  const [orderBy, setOrderBy] = React6.useState(void 0);
		  const [orderDir, setOrderDir] = React6.useState("asc");
		  const [columns, setColumns] = React6.useState([]);
		  const [indexes, setIndexes] = React6.useState([]);
		  const loadSchemas = React6.useCallback(async () => {
		  ...
		  const loadTables = React6.useCallback(async (schema, force = false) => {
		  ...
		  const loadRows = React6.useCallback(async (options) => {
		  ...
		  const loadStructure = React6.useCallback(async (schema, table) => {
		    try {
		  ...
		  for (const schema of schemas) {
		    ...
		    tree.push(
		      React6.createElement(
		        ...
		        React6.createElement("span", { className: "dbm-tree-caret" }, isOpen ? "\u25BE" : "\u25B8"),
		        React6.createElement("span", { className: "dbm-tree-name" }, schema),
		        tables === void 0 ? null : React6.createElement("span", { className: "dbm-tree-meta" }, String(tables.filter((table) => matchesFilter2(table.name)).length))
		      )
		    );
		    if (!isOpen) continue;
		    if (loading && tables === void 0) {
		      tree.push(React6.createElement("div", { key: `loading-${schema}`, className: "dbm-tree-item dbm-tree-indent-1 dbm-hint" }, t("common.loading")));
		      continue;
		    }
		    ...
		    for (const table of list) {
		      tree.push(
		        React6.createElement(
		          ...
		          React6.createElement("span", { className: "dbm-tree-caret" }, table.type === "view" ? "\u25EB" : "\u25A4"),
		          React6.createElement("span", { className: "dbm-tree-name" }, table.name),
		          table.rows === void 0 ? null : React6.createElement("span", { className: "dbm-tree-meta" }, String(table.rows))
		        )
		      );
		    }
		  }
		  const left = React6.createElement(
		    "div",
		    { className: "dbm-side" },
		    React6.createElement(
		      "div",
		    ...
		    React6.createElement("div", { className: "dbm-side-body" }, tree)
		  );
		  const selection = activeSchema !== void 0 && activeTable !== void 0 ? { schema: activeSchema, table: activeTable } : void 0;
		  const body = [];
		  if (selection === void 0) {
		    body.push(React6.createElement(Empty, { key: "empty", message: t("db.selectTable") }));
		  } else {
		    body.push(
		      React6.createElement(TabStrip, {
		        key: "tabs",
		        active: tab,
		        tabs: [
		    ...
		    if (tab === "browse") {
		      body.push(React6.createElement(BrowseTab, {
		        key: "browse-body",
		        rows,
		        orderBy,
		        orderDir,
		    ...
		    if (tab === "structure") {
		      body.push(React6.createElement(StructureTab, { key: "structure-body", columns, indexes }));
		    }
		    if (tab === "sql") {
		      body.push(
		        React6.createElement(SqlTabView, {
		          key: "sql-body",
		          source,
		          schema: activeSchema,
		          api,
		    ...
		    if (tab === "search") {
		      body.push(
		        React6.createElement(SearchTab, {
		          key: "search-body",
		          onSearch: (payload) => {
		            void loadRows({
		    ...
		    if (tab === "insert") {
		      body.push(
		        React6.createElement(InsertTab, {
		          key: "insert-body",
		          api,
		          source,
		          schema: selection.schema,
		  ...
		  return React6.createElement(
		    ...
		    React6.createElement(
		      "div",
		      { className: "dbm-header" },
		      React6.createElement(BackButton, { onBack, label: t("panel.backToList") }),
		      React6.createElement("span", { className: "dbm-title" }, source.name),
		      React6.createElement("span", { className: `dbm-badge dbm-badge-${source.kind}` }, source.kind),
		      React6.createElement("span", { className: "dbm-subtitle dbm-mono" }, source.kind === "sqlite" ? source.file ?? "" : `${source.host ?? ""}:${source.port ?? ""}`),
		      React6.createElement("span", { className: "dbm-spacer" }),
		      React6.createElement(BackButton, { onBack: onClose })
		    ),
		    error === void 0 ? null : React6.createElement(ErrorBanner, { message: error }),
		    notice === void 0 ? null : React6.createElement("div", { className: "dbm-ok", style: { padding: "6px 14px" } }, notice),
		    React6.createElement("div", { className: "dbm-split" }, left, React6.createElement("div", { className: "dbm-main" }, body))
		  );
		}
		function emptyPage() {
		...
		function BrowseTab(props) {
		  ...
		  if (rows === void 0) return React6.createElement(Empty, { message: t("common.loading") });
		  if (rows.error !== void 0) return React6.createElement(ErrorBanner, { message: rows.error });
		  ...
		  return React6.createElement(
		    ...
		    React6.createElement(
		      ...
		      React6.createElement("button", { type: "button", className: "dbm-btn dbm-btn-sm", onClick: onRefresh }, t("browse.refresh")),
		      React6.createElement("label", { className: "dbm-hint" }, `${t("browse.pageSize")}: ${""}`),
		      React6.createElement(
		        "select",
		        {
		          className: "dbm-select",
		        ...
		        [50, 100, 200, 500, 1e3].map((size) => React6.createElement("option", { key: size, value: String(size) }, String(size)))
		      ),
		      React6.createElement("span", { className: "dbm-hint" }, t("browse.total", { n: page.total })),
		      React6.createElement("span", { className: "dbm-spacer" }),
		      rows.loading ? React6.createElement("span", { className: "dbm-hint" }, t("common.loading")) : null
		    ),
		    page.columns.length === 0 ? React6.createElement(Empty, { message: t("browse.empty") }) : React6.createElement(
		      "div",
		      { className: "dbm-data" },
		      React6.createElement(
		        "table",
		        ...
		        React6.createElement(
		          ...
		          null,
		          page.rows.map(
		            (row, index) => React6.createElement(
		              "tr",
		              { key: index },
		              columns.map(
		                (column) => React6.createElement(
		    ...
		    React6.createElement(
		      "div",
		      { className: "dbm-pager" },
		      React6.createElement("button", { type: "button", className: "dbm-btn dbm-btn-sm", disabled: page.page <= 1, onClick: () => onPage(page.page - 1) }, t("browse.prev")),
		      React6.createElement("span", null, t("browse.page", { page: page.page, pages })),
		      React6.createElement("button", { type: "button", className: "dbm-btn dbm-btn-sm", disabled: page.page >= pages, onClick: () => onPage(page.page + 1) }, t("browse.next")),
		      page.primaryKey.length === 0 ? React6.createElement("span", { className: "dbm-hint" }, t("browse.noPk")) : null
		    )
		  );
		}
		function StructureTab(props) {
		  ...
		  return React6.createElement(
		    ...
		    React6.createElement(
		      "div",
		      { className: "dbm-scroll" },
		      React6.createElement("div", { className: "dbm-pad" }, React6.createElement("strong", null, t("structure.columns"))),
		      React6.createElement(
		        "table",
		        { className: "dbm-table" },
		        React6.createElement(
		        ...
		        React6.createElement(
		          ...
		          columns.map(
		            (column) => React6.createElement(
		              "tr",
		              { key: column.name },
		              React6.createElement("td", { className: "dbm-mono" }, column.name),
		              React6.createElement("td", { className: "dbm-mono" }, column.type),
		              React6.createElement("td", null, column.nullable ? t("common.yes") : t("common.no")),
		              React6.createElement("td", null, column.key === "" ? t("common.none") : column.key),
		              React6.createElement("td", { className: "dbm-mono" }, column.defaultValue ?? t("common.none")),
		              React6.createElement("td", { className: "dbm-mono" }, column.extra ?? t("common.none")),
		              React6.createElement("td", null, column.comment ?? "")
		            )
		          )
		        )
		      ),
		      React6.createElement("div", { className: "dbm-pad" }, React6.createElement("strong", null, t("structure.indexes"))),
		      indexes.length === 0 ? React6.createElement("div", { className: "dbm-pad dbm-hint" }, t("structure.noIndexes")) : React6.createElement(
		        "table",
		        { className: "dbm-table" },
		        React6.createElement(
		          "thead",
		        ...
		        React6.createElement(
		          ...
		          null,
		          indexes.map(
		            (index) => React6.createElement(
		              "tr",
		              { key: index.name },
		              React6.createElement("td", { className: "dbm-mono" }, index.name),
		              React6.createElement("td", null, index.unique ? t("common.yes") : t("common.no")),
		              React6.createElement("td", { className: "dbm-mono" }, index.columns.join(", ")),
		              React6.createElement("td", null, index.type ?? t("common.none"))
		            )
		          )
		        )
		      )
		...
		function SqlTabView(props) {
		  ...
		  const [sql, setSql] = React6.useState("");
		  const [allowWrite, setAllowWrite] = React6.useState(false);
		  const [result, setResult] = React6.useState(void 0);
		  ...
		  return React6.createElement(
		    ...
		    React6.createElement(
		      ...
		      React6.createElement(
		        "div",
		        { className: "dbm-row" },
		        React6.createElement("button", { type: "button", className: "dbm-btn dbm-btn-primary", disabled: busy, onClick: () => {
		          void run();
		        ...
		        React6.createElement("span", { className: "dbm-hint" }, t("sql.allowWrite.hint"))
		      )
		    ),
		    result === void 0 ? null : React6.createElement(
		      "div",
		      { className: "dbm-tab-body", style: { minHeight: 0 } },
		      React6.createElement("div", { className: "dbm-pad dbm-hint" }, result.message),
		      result.columns.length === 0 ? null : React6.createElement(
		        "div",
		        { className: "dbm-data" },
		        React6.createElement(
		          "table",
		          null,
		          React6.createElement("thead", null, React6.createElement("tr", null, result.columns.map((column) => React6.createElement("th", { key: column }, column)))),
		          React6.createElement(
		            "tbody",
		            null,
		            result.rows.map(
		              (row, index) => React6.createElement(
		                ...
		                { key: index },
		                row.map(
		                  (cell, cellIndex) => React6.createElement(
		                    "td",
		                    { key: cellIndex, className: isNull(cell) ? "dbm-null" : void 0, title: renderCell(cell) },
		                    renderCell(cell)
		                  )
		...
		function SearchTab(props) {
		  ...
		  const [condition, setCondition] = React6.useState("");
		  const submit = () => {
		  ...
		  return React6.createElement(
		    ...
		    React6.createElement(
		      ...
		      React6.createElement(
		        ...
		        React6.createElement("button", { type: "button", className: "dbm-btn dbm-btn-primary", onClick: submit }, t("search.run"))
		      ),
		      React6.createElement(
		        "div",
		        { className: "dbm-row" },
		        React6.createElement("span", { className: "dbm-hint" }, t("search.condition")),
		        React6.createElement("input", {
		          className: "dbm-input dbm-mono",
		          style: { flex: 1 },
		          value: condition,
		    ...
		    rows === void 0 ? null : rows.error !== void 0 ? React6.createElement(ErrorBanner, { message: rows.error }) : React6.createElement(
		      "div",
		      { className: "dbm-tab-body", style: { minHeight: 0 } },
		      React6.createElement("div", { className: "dbm-pad dbm-hint" }, t("browse.total", { n: rows.page.total })),
		      React6.createElement(
		        "div",
		        { className: "dbm-data" },
		        React6.createElement(
		          "table",
		          null,
		          React6.createElement("thead", null, React6.createElement("tr", null, rows.page.columns.map((column) => React6.createElement("th", { key: column.name }, column.name)))),
		          React6.createElement(
		            "tbody",
		            null,
		            rows.page.rows.map(
		...
		function InsertTab(props) {
		  ...
		  const [useNull, setUseNull] = React6.useState({});
		  const [busy, setBusy] = React6.useState(false);
		  const submit = async () => {
		    const payload = [];
		  ...
		  return React6.createElement(
		    ...
		    React6.createElement(
		      ...
		      React6.createElement(
		        "div",
		        { className: "dbm-pad" },
		        React6.createElement("strong", null, `${t("insert.title")} \u2014 ${table}`),
		        React6.createElement("div", { className: "dbm-hint" }, t("insert.hint")),
		        React6.createElement(
		          "div",
		          ...
		          columns.map(
		            (column) => React6.createElement(
		              ...
		              React6.createElement(
		                ...
		                null,
		                column.name,
		                React6.createElement("span", { className: "dbm-hint" }, ` \xB7 ${column.type}${column.nullable ? "" : " NOT NULL"}${column.key === "PRI" ? " PK" : ""}`)
		              ),
		              React6.createElement("input", {
		                className: "dbm-input dbm-mono",
		              ...
		              column.nullable ? React6.createElement(
		                "span",
		                { className: "dbm-check" },
		                React6.createElement("input", {
		                  type: "checkbox",
		...
		function DatabasePanel(props) {
		  ...
		  const [snapshot, setSnapshot] = React7.useState(() => controller.getSnapshot());
		  const [sources, setSources] = React7.useState(void 0);
		  const [settings, setSettings] = React7.useState({ allowAgentWrite: false, requireApproval: true });
		  const [engines, setEngines] = React7.useState([]);
		  const [error, setError] = React7.useState(void 0);
		  void localeTick;
		  React7.useEffect(() => controller.subscribe(() => setSnapshot(controller.getSnapshot())), [controller]);
		  const reload = React7.useCallback(async () => {
		    try {
		  ...
		  const saveGate = React7.useCallback(async (patch) => {
		    try {
		  ...
		  const connect = React7.useCallback(async (source) => {
		    try {
		  ...
		  let body;
		  if (screen.name === "sql") {
		    body = React7.createElement(SqlDatabaseView, {
		      api,
		    ...
		    body = React7.createElement(RedisDatabaseView, {
		      api,
		      source: screen.source,
		    ...
		    body = React7.createElement(Empty, { message: t("common.loading") });
		  } else {
		    body = React7.createElement(SourceListView, {
		      api,
		      sources,
		      settings,
		      engines,
		    ...
		    try {
		      return container.inject(
		        ...
		        () => container.register(
		          ...
		          (props) => React8.createElement("span", {
		    ...
		    try {
		      return container.inject(
		        ...
		        () => container.register(
		          ...
		          () => React8.createElement(PanelHost, { controller, api, localeListeners, onClose: close })
		        )
		      );
		    } catch (failure) {
		...
		function PanelHost(props) {
		  ...
		  const [localeTick, setLocaleTick] = React8.useState(0);
		  React8.useEffect(() => {
		  ...
		  return React8.createElement(DatabasePanel, { controller, api, localeTick, onClose });
		}
