...
function toSpec(column) {
  return {
    ...
    ...column.defaultValue === void 0 ? {} : { defaultValue: unquoteNumericDefault(column.defaultValue) },
...
function unquoteNumericDefault(value) {
  ...
  "src/drivers/mysql.ts"() {
    ...
    MysqlDriver = class {
      ...
      async columns(schema, table) {
        ...
        const [rows] = await (await this.open()).query({
          sql: "SELECT COLUMN_NAME AS name, COLUMN_TYPE AS type, IS_NULLABLE AS nullable, COLUMN_DEFAULT AS dflt, COLUMN_KEY AS col_key, COLUMN_COMMENT AS comment, EXTRA AS extra, COLLATION_NAME AS collation FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? ORDER BY ORDINAL_POSITION",
          values: [target, table]
        });
  ...
  "src/drivers/sqlite.ts"() {
    ...
    SqliteDriver = class {
      ...
      async insertRows(schema, table, rows) {
        ...
        const names = columns.map((name2) => requireIdentifier(name2, "column name", quoteSqlite)).join(", ");
      ...
      async alterColumn(schema, table, spec, options = {}) {
        ...
        const onlyRenamed = newName !== spec.name && normalizeType(next.type, "sqlite") === normalizeType(current.type, "sqlite") && next.nullable === current.nullable && normalizeDefault(next.defaultValue, "sqlite") === normalizeDefault(current.defaultValue, "sqlite") && next.unique === current.unique && current.primaryKeyPosition === void 0;
        if (onlyRenamed) {
          const qualified = qualifySqlite(schema, table);
          return this.exec(
...
var COLUMN_ATTRIBUTES = ["unsigned", "zerofill", "binary", "onUpdateCurrentTimestamp"];
var INDEX_KINDS = ["primary", "unique", "index", "fulltext", "spatial"];
var MAINTENANCE_OPS = ["check", "optimize", "repair", "analyze"];
